...
| RSK-0008 | DEV-0202 requires contradictory encryption-indicator handling but approves only `Manifest.plist.IsEncrypted` as a signal | A fabricated secondary signal would create an unsupported forensic classification rule | No secondary signal was invented; implemented Boolean/missing/malformed handling is tested; task remains IN_PROGRESS | OPEN — HIGH | Owner must approve a second precise indicator or remove/defer the contradictory-indicators fixture |
