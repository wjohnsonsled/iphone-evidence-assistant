...
| RSK-0007 | Approved DEV-0202 identity and corruption rules assign different outcomes to a present but invalid SQLite `Manifest.db` | Deterministic precedence could misclassify non-Apple input as corrupt or corrupt Apple-like input as non-Apple | Stage-B classifier implementation paused; conflict recorded in DEC-0007 and acceptance criteria | OPEN — HIGH | Owner must select the controlling outcome or define an additional identity observation that distinguishes the cases |
